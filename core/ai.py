...
def get_active_model_config():
    """
    获取当前启用的 AI 模型配置
    返回: (model_name, api_key, is_deepseek)
    """
    # 读取所有可能的配置
    deepseek_enabled = cfg.get("deepseek.enabled")
    deepseek_key = cfg.get("deepseek.api_key")
    minimax_enabled = cfg.get("minimax.enabled")
    minimax_key = cfg.get("minimax.api_key")

    # 检查 DeepSeek 是否启用
    ds_enabled = os.environ.get("DEEPSEEK_ENABLED") or deepseek_enabled
    mm_enabled = os.environ.get("MINIMAX_ENABLED") or minimax_enabled

    # 优先使用 DeepSeek
    if str(ds_enabled).lower() == "true":
        api_key = os.environ.get("DEEPSEEK_API_KEY") or deepseek_key
        if api_key:
            return ("deepseek", api_key, True)

    # 检查 MiniMax
    if str(mm_enabled).lower() == "true":
        api_key = os.environ.get("MINIMAX_API_KEY") or minimax_key
        if api_key:
            return ("minimax", api_key, False)

    # 如果都没有启用，尝试使用默认的 MiniMax
    api_key = os.environ.get("MINIMAX_API_KEY") or minimax_key
    if api_key:
        return ("minimax", api_key, False)
...
def analyze_article(title: str, content: str) -> dict:
    """
    调用 AI 大模型接口（支持 DeepSeek 和 MiniMax），对文章进行结构化分析打标。
    分类为：产品功能、运营活动、其他。
    ...
    try:
        # 获取当前启用的模型配置
        model_name, api_key, is_deepseek = get_active_model_config()

        if not api_key:
            print_error("[AI] 未检测到有效的 AI API Key。请在配置页启用并配置对应的模型。")
...
"""
        ...
        if is_deepseek:
            # DeepSeek API 调用（使用 OpenAI SDK）
            print_info(f"[AI] 正在使用 DeepSeek 给文章 '{title[:30]}' 打标...")
            ...
            if response.status_code != 200:
                print_error(f"[AI] MiniMax API 调用失败, 状态码: {response.status_code}, 详情: {response.text}")
